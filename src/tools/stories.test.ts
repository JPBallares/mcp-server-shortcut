...
describe("StoryTools", () => {
	...
	describe("create method", () => {
		test("should register the correct tools with the server", () => {
			...
			expect(mockTool.mock.calls?.[2]?.[0]).toBe("get-story-with-related");
			expect(mockTool.mock.calls?.[3]?.[0]).toBe("search-stories");
			expect(mockTool.mock.calls?.[4]?.[0]).toBe("search-stories-with-related");
			...
			expect(mockTool.mock.calls?.[13]?.[0]).toBe("add-external-link-to-story");
			expect(mockTool.mock.calls?.[14]?.[0]).toBe("remove-external-link-from-story");
			expect(mockTool.mock.calls?.[15]?.[0]).toBe("get-stories-by-external-link");
			expect(mockTool.mock.calls?.[16]?.[0]).toBe("get-stories-by-external-link-with-related");
			expect(mockTool.mock.calls?.[17]?.[0]).toBe("set-story-external-links");
		});
	});
